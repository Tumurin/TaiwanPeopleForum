...
export default defineEventHandler(async (event) => {
    console.log("[server api auth/login] 請求登入");
    ...
    console.log("[server api auth/login] 解析帳號密碼：", account, password)
    ...
    console.log("[server api auth/login] 資料庫查找用戶：", user)
    ...
    console.log("[server api auth/login] 檢查密碼是否正確：", isPasswordMatched)
    ...
    console.log("[server api auth/login] 登記 refresh token 到資料庫", refreshToken)
    ...
    console.log("[server api auth/login] 儲存 Cookie", getCookie(event, "refresh_token"));
